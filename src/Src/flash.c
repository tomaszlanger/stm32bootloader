...
flash_status flash_erase(uint32_t address)
{
  ...
  flash_status status = FLASH_STATUS_ERROR;
  ...
  erase_init.NbPages = ((FLASH_END - address) / FLASH_PAGE_SIZE) + 1;
...
flash_status flash_write(uint32_t address, uint32_t *data, uint32_t length)
{
  ...
  for (uint32_t i = 0u; (i < length) && (FLASH_STATUS_OK == status); i++)
  {
...
/**
 * @brief   This function raad the memory.
 * @param   address: First address to be read to.
 * @param   *data:   Array of the data that we want to write readed data.
 * @param   *length: Size of the read array.
 * @return  status: Report about the success of the read.
 */
...
{
	pFunction appEntry;		
	appEntry = (pFunction)(uint32_t*)(FLASH_APP_START_ADDRESS + VECTOR_TABLE_SIZE + 1); 
